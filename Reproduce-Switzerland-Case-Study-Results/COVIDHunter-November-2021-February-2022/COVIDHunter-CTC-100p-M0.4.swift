...
let NUM_DAYS = 790 // maximum number of days simulated (2 years)
...
//CRW
//let X = 0.04288 //the hospitalizations-to-cases ratio, for CRW-100% = 4.288% for CTC-100% = 2.780%
//let Y = 0.0273  //the deaths-to-cases ratio, for CRW-100% = 2.730% for CTC-100% = 1.739%



let R0_INTRINSIC = 2.7 //https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1008031#pcbi.1008031.s001
let R0_INTRINSIC_VARIANT1 = 5.0
...
let IMPORTED_MUTATION_RATE = 70    // Percentage of imported cases that are of mutated variant 1 (as of FIRST_VARIANT_DAY).  Set to 0 to disable mutations
let FIRST_VARIANT_DAY = 535  // First day a traveller can bring in the mutated virus
let R0_ASYMPTOMATIC = 2.0
let R0_ASYMPTOMATIC_VARIANT1 = 3.0

// Vaccination parameters
let ENABLE_VACCINATIONS = true
let FIRST_VACCINATION_DAY = 424 // note:  the model assumes that you gain immunity immediately after vaccination.   Starting 2 weeks later accounts for the fact that in reality you need to wait 2 weeks for partial immunity.
let VACCINATION_RATE = 0.28     // Percentage of population vaccinated per day
...
// This is to fit the curve with 100%
let M =           [0, 0.45, 0.7,  0.7, 0.65, 0.63, 0.5, 0.355, 0.6, 0.7, 0.7, 0.71, 0.73, 0.73, 0.56, 0.45, 0.29, 0.25, 0.29, 0.45, 0.4, 0.4, 0.4 ]
let TRANSITIONS = [1,   58,  77,  118,  132,  151, 174,   245, 284, 303, 320,  330,  356,  387,  425,  475,  517,  577,  580, 606, 625, 680, 730 , 9999] // Day from start of sim that we move to next pandemic 'phase'. Last entry should be 9999

// This is to fit the curve with 50%
//let M =           [0, 0.35, 0.66,  0.7, 0.65, 0.63, 0.5, 0.36, 0.51, 0.65, 0.68, 0.69, 0.69, 0.7, 0.58, 0.4, 0.2, 0.2, 0.2, 0.3, 0.28, 0.4, 0.3 ]
//let TRANSITIONS = [1,   58,   76,  100,  132,  151, 174,  235,  274,  303,  320,  330,  356, 387, 420, 450, 500, 577,  608,  617,  630, 690, 730 , 9999] // Day from start of sim that we move to next pandemic 'phase'. Last entry should be 9999
...
// This is to fit the curve with 100%
//let M =           [0, 0.45, 0.7,  0.7, 0.65, 0.63, 0.5, 0.36, 0.37, 0.6, 0.71, 0.71, 0.71, 0.73, 0.73, 0.6, 0.55, 0.35, 0.3, 0.31, 0.48, 0.4, 0.4, 0.4]
//let TRANSITIONS = [1,   58,  77,  118,  132,  151, 174,  245,  260, 284,  303,  320,  330,  356,  387, 420,  470, 500, 577,  608,  617,  630, 690,  730, 9999] // Day from start of sim that we move to next pandemic 'phase'. Last entry should be 9999
// This is to fit the curve with 50%
//let M =           [0, 0.35, 0.7, 0.71, 0.65, 0.63, 0.5, 0.35, 0.5, 0.68, 0.7, 0.71, 0.71, 0.71, 0.54, 0.5, 0.25, 0.22, 0.3, 0.3, 0.4, 0.1, 0.4]
//let TRANSITIONS = [1,   58,  76,  100,  132,  151, 174,  235, 274,  303, 320,  330,  356,  387,  420,  470, 500, 577,  608,  617,  630, 690,  730, 9999] // Day from start of sim that we move to next pandemic 'phase'. Last entry should be 9999
...
print("Day Uninfected Daily_Cases Daily_Cases_(Variant_1) Daily_Cases_(Variant_2) Daily_Hospitalizations Daily_Deaths Active_Cases Contagious Immune Total_Vaccinated Asymptomatic_Cases Travelers R0 Ce(t) M(t) R0*(1-M(t)) R0*Ce(t) R(t)=R0*Ce(t)*(1-M(t)) Observed_R(t)")
...
func process_people(day: Int, phase: Int, CRWfactor: Double) -> Bool {
    ...
    for i in sick_person_ptr..<free_person_ptr {
        ...
        case State.CONTAGIOUS:
            ...
            contagious+=1
            // determine number of people to spread the virus to
    ...
    let cases_number = String(format: "%2.3f", Double(newly_infected[0]+newly_infected[1]))
    ...
    print("\(day+1) \(POPULATION-free_person_ptr) \(cases_number) \(newly_infected[0]) \(newly_infected[1]) \(hospitalizations_number) \(deaths_number) \(infected_original) \(contagious) \(sick_person_ptr+immune) \(total_vaccinated)  \(asymptomatic) \(total_travelers) \(intrinsic_r_string) \(C_string) \(M_string) \(phase_string) \(temp_r_string) \(phase_temp_r_string) \(actual_r_string)")
